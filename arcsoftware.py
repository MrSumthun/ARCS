...
def build_ui():
	...
	try:
		...
		style.configure('Treeview', background=PANEL_BG, fieldbackground=PANEL_BG, foreground=FG, rowheight=26, font=body_font)
	...
	def clear_current():
		"""Clear the current quote from the UI without creating a new one."""
		current['quote'] = None
		update_tree()
		update_totals()
		po_entry_var.set('')
	...
	def update_tree():
		...
		for idx, it in enumerate(q['items'], start=1):
			tree.insert('', 'end', iid=str(idx-1), values=(
				it.get('part_number'),
				it.get('description'),
				it.get('quantity'),
	...
	def delete_item():
		"""Delete the selected item from the current quote."""
		sel = tree.selection()
		if not sel:
	...
	def edit_item():
		"""Edit the selected item via a modal dialog."""
		sel = tree.selection()
		if not sel:
	...
	def load_quote():
		...
		def _export():
			...
			idx = int(sel[0])
			q = quotes[idx]
			fn = filedialog.asksaveasfilename(defaultextension='.json', filetypes=[('JSON files','*.json'), ('All files','*.*')], initialfile=f"quote-{q.get('id')}.json")
			if not fn:
				return
			with open(fn, 'w') as f:
	...
	def export_pdf():
		...
		try:
			...
			html.append('<tr><th>Part</th><th>Description</th><th>Qty</th><th>Unit</th><th>List</th><th>Source</th><th>Line</th></tr>')
			for it in q.get('items', []):
				html.append(f"<tr><td>{it.get('part_number')}</td><td>{it.get('description')}</td><td>{it.get('quantity')}</td><td>{it.get('unit_cost'):.2f}</td><td>{it.get('list_price'):.2f}</td><td>{it.get('source')}</td><td>{it.get('line_total'):.2f}</td></tr>")
			html.append('</table>')
		...
		for it in q.get('items', []):
			line = f"{it.get('part_number')} | {it.get('description')} | {it.get('quantity')} x {it.get('unit_cost'):.2f} = {it.get('line_total'):.2f}"
	...
	notes_text = tk.Text(notes_frame, wrap='word', height=5, bg=PANEL_BG, fg=FG, insertbackground=FG, relief='flat', bd=0, font=body_font, highlightthickness=1, highlightbackground='#2a2a2a')

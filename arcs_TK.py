...
##########
# DEPRECATED: Transfer to ARCS_qt.py and remove Tkinter support in future versions
# This file will remain for backward compatibility for now.
##########
USE_TK = not any(arg in ("--qt", "--pyqt") for arg in sys.argv)
if USE_TK:
...
if __name__ == "__main__":
    ...
    # Launch a PyQt build if requested via command-line flag (keeps backward compatibility)
    if "--qt" in sys.argv or "--pyqt" in sys.argv:
        try:
            from arcs import main as qt_main

            qt_main()
        except Exception:
            logger.exception("Failed to launch PyQt UI")
            # Only fall back to Tkinter if we actually imported Tk earlier
            if USE_TK:
                logger.info("Falling back to Tkinter UI")
                main()
            else:
                raise
    else:
